...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!('IntersectionObserver' in window)) {
    ...
    // Nothing should ever stay hidden if the observer misfires
  ...
  const map = new Map();
  ['about','focus','projects','skills','experience','certs','contact'].forEach(id => {
  ...
  document.querySelectorAll('a[href^="#"]').forEach(a => {
    a.addEventListener('click', function (e) {
      ...
      if (!t) return;
      e.preventDefault();
      const top = t.getBoundingClientRect().top + window.pageYOffset - 76;
      window.scrollTo({ top, behavior: reduceMotion ? 'auto' : 'smooth' });
    });
  });

  /* ---- Lightbox (certificates + screenshots) ---- */
  ...
  document.querySelectorAll('[data-cert]').forEach(btn => {
    btn.addEventListener('click', () => openLb(btn.dataset.cert, btn.querySelector('img')?.alt));
  });
